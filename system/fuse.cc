...
#if 1
     size_t size1;
     if(chfs->write(ino,size,off,buf,size1) == chfs_client::OK) {
        fuse_reply_write(req, size1);
     } else {
        fuse_reply_err(req, ENOENT);
     }
    // Change the above line to "#if 1", and your code goes here
        ...
        mode_t mode)
{
    ...
    status = chfs->mkdir(parent,name,mode,ino);
     if(status == chfs_client::OK) {
        e.ino = ino;
        getattr(ino, e.attr);
        fuse_reply_entry(req, &e);
...
fuseserver_symlink(fuse_req_t req,const char *link, fuse_ino_t parent,
   ...
   struct fuse_entry_param e;
    ...
    chfs_client::inum ino;
    chfs_client::status status;
    status = chfs->symlink(link,parent,name,ino);
    if(status == chfs_client::OK) {
        e.ino = ino;
        getattr(ino, e.attr);
        fuse_reply_entry(req, &e);
